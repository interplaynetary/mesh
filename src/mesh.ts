/**
 * Mesh - Main API for graph database operations
 * Provides chainable interface for get, put, on, off operations
 */

import * as utils from "./utils"
import { type WireAPI } from "./wire-transport"
...
export interface MeshAPI {
  get: {
    (key: string | null, lex?: LexWithDot, cb?: (data: unknown) => void, _opt?: WireOptions): MeshAPI
    (
      key: string | null,
  ...
  off: (cb?: (data: unknown) => void) => MeshAPI | void
  user: () => UserInterface & MeshAPI
  wire: WireAPI
  SEA: typeof SEA
}

const Mesh = (opt?: MeshOptions | string | string[]): MeshAPI => {
  ...
  const api = (initCtxid?: string): MeshAPI => {
    let ctxid = initCtxid
    const get = (
      lex: LexFilter,
      soul: string,
    ...
    return {
      ...
      off: function (cb?: (data: unknown) => void): MeshAPI | void {
        if (!ctxid) {
          console.log("error please provide a key using get(key)")
          if (cb) cb(null)
          return
      ...
      user: () => {
        if (!(user as { get?: unknown }).get) {
          Object.assign(user, api())
            ; (user as UserInterface & MeshAPI).get = function (
              this: MeshAPI,
              keys: string | string[],
              lex?: LexFilter | ((data: unknown) => void),
              cb?: ((data: unknown) => void) | WireOptions,
        ...
        return user as UserInterface & MeshAPI
      },
