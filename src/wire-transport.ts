...
const WireTransport = (opt: WireTransportOptions): WireAPI => {
    ...
    // Client mode
        ...
        transport.onMessage((data) => handleMessage(data, "server")) // Using 'server' as ID for client mode usually
        ...
        const rawSend = (data: string) => {
            if (!transport.isConnected()) throw new Error("Transport not connected")
            transport.send(data).catch(error => console.error("Send error:", error))
        }

        // Send HELLO to server/peer
        const sendHello = () => {
            rawSend(JSON.stringify({ hello: { pub: selfId } }))
        }
        ...
        // Route via FingerTable if transport supports it
        const routedSend = async (data: string) => {
            if (!transport.isConnected()) return { err: "Transport not connected" }

            // Same routing logic as server...
            try {
                const msg = JSON.parse(data) as WireMessage
                let targetId: string | undefined
                if (msg.get && msg.get["#"]) targetId = msg.get["#"]
    ...
    // Fallback
        ...
        sendFunction = () => ({ err: "No transport configured" })
    }
